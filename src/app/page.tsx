...
import { Carousel, ExerciseCard, MetaBadges, SectionTitle, SeeAll } from "@/components/ui";
    ...
    <>
      <section className="mx-auto grid max-w-6xl grid-cols-1 items-center gap-6 px-4 pb-6 pt-6 md:grid-cols-[1.1fr_1fr] md:gap-8 md:py-16">
        <div className="animate-rise">
          <p className="eyebrow text-amber-300">{SITE.gym} workout guide</p>
          <h1 className="display mt-3 text-[2.75rem] sm:text-6xl lg:text-7xl">
            See every rep.
            <br />
            <span className="text-red-500">Know every muscle.</span>
          </h1>
          <p className="mt-4 max-w-md text-[1.0625rem] leading-relaxed text-zinc-300 md:mt-5">
          ...
          <div className="mt-6 hidden items-center gap-5 text-meta text-zinc-400 md:mt-8 md:flex">
          ...
          {REGIONS.map((region) => {
            ...
            return (
              <Link
                ...
                className="surface group rounded-[1.25rem] px-4 py-3.5 transition-transform duration-300 ease-spring hover:bg-white/[0.07] active:scale-95"
              >
                <span className="display block text-xl group-hover:text-red-400 sm:text-2xl">{region}</span>
                <span className="mt-1 block text-meta text-zinc-400">{count} exercises</span>
              </Link>
            );
          })}
        </div>
        ...
        <Carousel item="basis-[78%] sm:basis-[45%]" grid="md:grid-cols-2 lg:grid-cols-4">
          {PROGRAMS.map((p) => (
            ...
            >
              ...
              <MetaBadges items={[p.level, `${p.daysPerWeek}× / week`]} />
              <h3 className="display mt-3 text-2xl">{p.name}</h3>
              <p className="mt-2 line-clamp-3 text-md text-zinc-300">{p.tagline}</p>
              <div className="mt-auto flex items-center gap-1 pt-4 text-sm font-bold text-amber-300">
                {p.days.length} workouts <ChevronRight size={16} />
              </div>
            </Link>
          ))}
        </Carousel>
